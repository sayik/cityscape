...
cityscape is operated as a single, continuously deployed service. Only the
latest released version (currently `1.x`) and the live hosted endpoint receive
security fixes.

## Security model

cityscape is intentionally small in attack surface:
...
- **Keyless.** The public API and the hosted MCP server
  (`https://mcp.cityscape.dev/mcp`, streamable HTTP) require no API key and
  store no per-user credentials. There is no user account system and no secret
  to leak on the client side.
- **No personal data.** cityscape proxies and normalizes public open data
...
- **SSRF protection.** The MCP client resolves only to the configured cityscape
...
`https://cityscape.dev` (API) and `https://mcp.cityscape.dev` (MCP).
